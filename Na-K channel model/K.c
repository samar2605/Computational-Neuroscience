...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 K D:/Sem 6/Theoritical and Computational Neuroscience/Assignments/Na-K channel model/K.mod\n");
...
static int _reset;
static char *modelname = "K channel for HH model";
...
static int  _f_rates (  double _lv ) {
   nalpha = ( 0.01 * ( _lv + 10.0 ) ) / ( exp ( ( _lv + 10.0 ) / 10.0 ) - 1.0 ) ;
   nbeta = 0.125 * exp ( _lv / 80.0 ) ;
 ...
 v=_v;
{
  ek = _ion_ek;
 { error =  states();
 if(error){fprintf(stderr,"at line 34 in file K.mod:\n    SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 } }}

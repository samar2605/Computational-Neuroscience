...
/*CVODE*/
 ...
 {
   rate ( _threadargscomma_ v ) ;
    l = l + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / tau)))*(- ( ( ( linf ) ) / tau ) / ( ( ( ( - 1.0 ) ) ) / tau ) - l) ;
   }
  return 0;
}
...
static double _nrn_current(double _v){double _current=0.;v=_v;{ {
   gcal = gcalbar * l * l * h2 ( _threadargscomma_ cai ) ;
 ...
 v=_v;
{
  ...
  cao = _ion_cao;
 { error =  state();
 if(error){fprintf(stderr,"at line 46 in file CaL.mod:\n	SOLVE state METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 } }}
